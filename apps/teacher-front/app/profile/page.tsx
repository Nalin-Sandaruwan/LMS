...
import { useAuth, useTeacherProfile } from "@/hooks/useAuth";
import type { TeacherProfile } from "@/components/profile/types";
...
export default function ProfilePage() {
    ...
    // Fetch the teacher's LMS record (fullName, shortBio, teachingExpert, etc.)
    const { data: teacher, isLoading: isTeacherLoading } = useTeacherProfile(user?.id);
    ...
    // Priority: LMS fullName → auth name → email prefix → fallback
    const displayName = teacher?.fullName || user?.name || user?.email?.split("@")[0] || "Teacher";
                ...
                <main className="pt-28 pb-20 container mx-auto">
                    ...
                    >
                        ...
                        <h1 className="text-4xl font-black text-gray-900 dark:text-white">
                            {`Welcome, ${displayName.split(" ")[0]} to your `}
                            <span
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            {activeTab === "profile" && (
                                <ProfileTab user={user} teacher={teacher} isLoading={isTeacherLoading} />
                            )}
                            {activeTab === "edit" && (
                                <EditProfileTab user={user} teacher={teacher} />
                            )}
